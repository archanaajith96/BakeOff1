 ...
 * Mod     : Make other squares dimmer without reducing their coverage.
 * Code    : Line 138 ~ 159
...
void drawButton(int i)
{
   ...
   * Line 138 ~ 159
   * By making the non-target squares very, very dim, we can allow the user 
   * to focus better on the target square with cyan coloring. This does not 
   * affect the accessability of any of the squares.
   */
  Rectangle bounds = getButtonLocation(i);

  if (trials.get(trialNum) == i) {
    // Draw Target Square (CYAN) 
    ...
    fill(200);
    rect(bounds.x + (0.25 * bounds.width), bounds.y + (0.25 * bounds.height), (0.5 * bounds.width), (0.5 * bounds.height));
  }
}
