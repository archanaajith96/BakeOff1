 ...
 * Mod     : Create a competing red dot to encourage faster movement by the user.
 * Code    : Line 19~30 && 103~117 && 153~197
...
// x,y distance between targets && x,y position of the current target square.
int distX, distY, currCenterX, currCenterY;
...
void draw()
{
  ...
  /* DAVID'S CODE MODIFICATION
   * Line 103 ~ 117
   * Draw red dot if still competing with the user.
   * Update the dot's position with the velocity and check whether it reached the target.
   */
  if (isCompeting) {
    fill(255,0,0,150);
...
void mousePressed() // test to see if hit was in target!
{
 ...
 //check to see if mouse cursor is inside button 
  ...
  if (trialNum > 0 && trialNum < 16) {
    ...
    int prevCenterX = prevBound.x + (int)(0.5 * prevBound.width);
    int prevCenterY = prevBound.y + (int)(0.5 * prevBound.height);
